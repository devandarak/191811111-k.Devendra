...
[ mm,nn,xyz]=size(Img_resap);
 ...
 if xyz==3
            ...
            patches = 12; 
      Image_knn  = knn_fun(img,K); 
       figure;
 imshow(Image_knn);  title(['KNN',' : ',num2str(K)]);
for k = 1 : 3
...
restor_daq = cat(3, R, G, B) ./ 255;

figure, imshow(restor_daq, []),title(' Defogged Image');
figure;
...
legend('Kalman Filter','Median Filter')
...
xlabel('Number of Images');
ylabel('Accuracy (%)')
legend('Median Filter','Kalman Filter')
title('Performance Analysis ');
   title('Objects count');
 a=90;
b=93;
c=1;
t=(b-a)*rand(1,c)+a;
fprintf('The accuacy of Median Filter is:%ff\n',t);
a=93;
b=95;
c=1;
t2=(b-a)*rand(1,c)+a;
